...
const Footer = () => {
  return (
    <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
      ...
      border-t-[1px] border-t-[#434A50]">
        <div className="flex md:flex-row flex-col justify-between items-center">
          <object type="image/svg+xml" data={logo} className="icon" />
          <p className="font-diphylleia text-center md:text-[16px] text-[14px] leading-[27px] text-primary">
          © Saegil
          </p>
        </div>
        ...
        <div className="flex flex-row md:mt-0 mt-6">
          ...
          <Link to={social.link} className="text-primary font-diphylleia">
            {social.id}
            {/* <img 
              key={social.id}
              src={social.icon}
